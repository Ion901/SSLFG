        ...
        <form action="{{ route('posts.update', $post->post_slug) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="mb-4">
                ...
                <input type="text" id="title" name="title"
                    ...
                    value="{{ htmlspecialchars($post->post_title, ENT_QUOTES, 'UTF-8') }}">
            </div>
                ...
                <div class="combobox">
                    <div>
                        ...
                        <select id="competition_name" name="competition_name"
                            class="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
                            onchange="document.getElementById('competition_input').value=this.value">
                            <option value=""></option>
            ...
            <div class="competition">
                <div class="combobox2">
                    <div>
                        <label for="competition_location" class="block text-gray-700 font-bold mb-2">Locatia
                            competitiei</label>
                        <select id="competition_location" name="competition_location"
                            class="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
                            onchange="document.getElementById('competition_location_input').value=this.value">
                            <option value=""></option>
                            @foreach ($competitions as $competition)
            ...
            <div class="mb-4">
                ...
                <textarea type="text" id="content" name="content"
                    class="hidden w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500">{!! $post->post_content !!}</textarea>
            ...
            <div class="card p-4">
                ...
                <div class="row">
                    <div class="col">
                        <input id="photo1" name="photo[]" type="file" class="file" multiple
                            data-show-upload="true" data-show-remove="true" data-show-caption="true"
                ...
                @foreach ($post->images as $index => $image)
                    <div class="flex flex-col w-[300px]">
                        ...
                        <div class="bg-gray-500 p-2 flex flex-wrap items-end">
                            <form action="{{ route('posts.update', $post->post_slug) }}" method="POST"
                                ...
                                <button type="submit"
                                    class="w-fit bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded">
                                    Actualizează
                                </button>
                                <button type="button"
                                    class="clear-file w-fit bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 ml-2.5 rounded">
                                    Anuleaza
                                </button>
                            </form>
                            <form action="{{ route('posts.destroy', $post->post_slug) }}" method="POST"
                                id="delete">
                                @csrf
                                @method('DELETE')
                                <input type="hidden" name="imageId" value="{{ $image->id }}">
